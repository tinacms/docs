...
<imageEmbed image={{ src: "/fetch-option-1.png", alt: "Diagram of an existing page served statically from the build pipeline, falling back to the TinaCMS API on a branch mismatch" }} caption="Flow when an existing page is requested" />

<imageEmbed image={{ src: "/fetch-option-2.png", alt: "Diagram of a new page falling back to ClientFallbackPage and fetching data dynamically from the TinaCMS API" }} caption="Flow when a new page is requested" />
