...
```mermaid
sequenceDiagram
    participant Admin as Admin (parent window)
    participant Iframe as Editor iframe
    participant Middleware as tina() middleware
    participant Island as /tina-island/[name]

    Iframe->>Middleware: Page request (edit mode)
    Middleware-->>Iframe: HTML with bridge bootstrap and data-tina-form payloads
    Iframe->>Admin: open
    Note over Iframe: Bridge seeds its in-memory store
    loop Every edit
        Admin->>Iframe: updateData
        Iframe->>Island: POST current overlay
        Island-->>Iframe: Re-rendered HTML fragment
        Note over Iframe: Bridge swaps the fragment into the live DOM
    end
```
