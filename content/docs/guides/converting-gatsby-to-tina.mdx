...
<Callout
  variant="info"
  body={<>
    You may need to update other elements on your site. For unsupported markdown elements in TinaCMS, refer to [our guide](https://tina.io/docs/reference/types/rich-text/?#unsupported-elements).
  </>}
/>
...
<imageEmbed image={{ src: "/live preview date changes.gif", alt: "editing an article's date using the visual editor" }} caption="Article date updating in real time" />
...
<Callout variant="info"
  body={<>
    We don't need to use the `useTina` hook here because the homepage is static.
  </>}
/>
