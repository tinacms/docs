...
<recipe
  title="Rendering blocks with a Blocks component"
  description="The component receives the page, walks the blocks the editor arranged, and renders one component per block template."
  code="//�Blocks.tsx
  ...
  instruction={[
    {
      header: "Import a component per block template",
      itemDescription:
        "Each block template in the schema gets its own React component. Testimonial is imported here too, ready for when you add that case.",
      codeLineStart: 3,
      codeLineEnd: 8
    },
    {
      header: "Map over the blocks in order",
      itemDescription:
        "props.blocks holds the blocks the editor arranged, in their saved order. Guard against a page with no blocks, then map over them.",
      codeLineStart: 13,
      codeLineEnd: 14
    },
    {
      header: "Switch on __typename to pick the component",
      itemDescription:
        "Every block carries a __typename that identifies its template, so switching on it picks the right component. The default case returns null, which is what renders for a template you have not handled yet.",
      codeLineStart: 15,
      codeLineEnd: 36
    }
  ]}
