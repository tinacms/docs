...
<Callout
  variant="info"
  body={<>
    The `defineSchema` API has changed, too. Read on for how to upgrade
  </>}
/>
...
<Callout
  variant="info"
  body={<>
    We've previously assumed that `blocks` usage would _always_ be as an array. We'll be keeping that assumption with the `blocks` type for compatibility, but `object` will allow for non-array polymorphic objects.
  </>}
/>
...
<Callout
  variant="info"
  body={<>
    There is no typescript help for this feature for now
  </>}
/>
...
<Callout
  variant="warning"
  body={<>
    Sorting and filtering is still not supported for list queries.
  </>}
/>
