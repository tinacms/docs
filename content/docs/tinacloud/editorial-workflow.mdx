...
last_edited: '2026-09-21T01:37:13.523Z'
...
<Callout
  variant="info"
  body={<>
    The Editorial Workflow feature is available on Teams + plans and beyond.
  </>}
/>
...
sequenceDiagram
    ...
    participant Cloud as TinaCloud
    participant GitHub

    Editor->>Admin: Save while on a protected branch
    Admin->>Editor: Ask for a new branch name
    Editor->>Admin: New branch name
    Admin->>Cloud: Start the editorial workflow with the edit
    Cloud->>GitHub: Create the new branch
    Cloud->>Cloud: Index the new branch
    Cloud->>GitHub: Commit the changes and open a draft pull request
    Cloud-->>Admin: Branch name and pull request link
    Admin->>Editor: Now editing on the new branch
    Editor->>GitHub: Merge the pull request to publish
```
