...
<Callout variant="info"
  body={<>
    token, clientId and branch are **not** used in [local-mode](/docs/tinacloud/#local-mode). To setup these values for production see [this doc](/docs/tinacloud/overview)
  </>}
/>
...
```mermaid
flowchart LR
    Client[Generated TinaCMS client query] --> Mode{Local mode or production mode}
    Mode -->|Local| Dev[Content API served by tinacms dev on port 4001]
    Dev --> Files[Your content files on disk]
    Mode -->|Production| Cloud[TinaCloud Content API for your client ID and branch]
    Cloud --> Git[Indexed content from your Git repo]
    Header[x-branch request header] -->|Swaps the branch in the API URL| Cloud
```
  ...
  cards={[
    {
      ...
      description: "Write advanced GraphQL queries",
      link: "/docs/data-fetching/custom-queries/",
      linkText: "Read more"
    },
    {
    ...
    },
    {
      ...
      description: "Define your content structure",
      link: "/docs/schema/",
      linkText: "Read more"
    },
    {
      ...
      link: "/docs/graphql/queries/advanced/filter-documents/",
      linkText: "Read more"
    }
  ]}
/>
