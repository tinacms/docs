...
<Callout
  variant="info"
  body={<>
    For advanced use-cases, you can also manually query the underlying [GraphQL API](/docs/data-fetching/custom-queries/#writing-inline-queries).
  </>}
/>
...
<Callout variant="info"
  body={<>
    When using the auto-generated &lt;collection-name&gt;Connection, the query will only apply to the document's fields, not the `_sys` properties. Filtering in code as shown above is a suitable alternative.
  </>}
/>
